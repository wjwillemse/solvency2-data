...
class TestReadRFR(unittest.TestCase):
    def test_read_input_date(self):
        ...
        self.assertEqual(
            type(actual),
            type(expected),
            "Read function, input_date: returned types not matching",
        )
        self.assertEqual(
            actual, expected, "Read function, input_date: returned values not matching"
        )
    ...
    def test_read_reference_date(self):
        ...
        self.assertEqual(
            type(actual),
            type(expected),
            "Read function, reference_date: returned types not matching",
        )
        self.assertEqual(
            actual,
            expected,
            "Read function, reference_date: returned values not matching",
        )
    ...
    def test_read_meta_date(self):
        ...
        expected = pd.Series(
            ...
            # 'reference date'],
            data=[
                "EUR_31_12_2017_SWP_LLP_20_EXT_40_UFR_4.2",
                1,
                20,
                40,
                4.2,
        ...
        self.assertEqual(
            type(actual),
            type(expected),
            "Read function, meta data: returned types not matching",
        )
        assert (
            actual == expected
        ).all(), "Read function, meta data: returned values not matching"
    ...
    def test_read_spot_rates(self):
        ...
        self.assertEqual(
            type(actual),
            type(expected),
            "Read function, spot rates: returned types not matching",
        )
        assert (
            actual == expected
        ).all(), "Read function, spot rates: returned values not matching"


class TestEiopaDB(unittest.TestCase):
    def test_db_spot_rates(self):
        """Test of read spot rates function"""
        ...
        # Actual output
        workspace = {
            "database": "tests\\test_data\\eiopa.db",
            "raw_data": "tests\\test_data\\raw",
        }
        df = get(date.date(), data_type="rfr", workspace=workspace)
        df = df.loc[(df.currency_code == "EUR") & (df.scenario == "base")]
        df = df.set_index("duration")["spot"].rename("Euro")
        actual = df
...
class TestSmithWilson(unittest.TestCase):
    def test_big_h(self):
        ...
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="big_h output not matching"
        )
    ...
    def test_big_g_1(self):
        ...
        # Input
        alfa = 1.1
        q = np.array([[1, 0, 0, 0], [0, 1.01, 0, 0], [0, 0, 1.02, 0], [0, 0, 0, 1.03]])
        t2 = 1
        tau = 0.9
        nrofcoup = 1
        ...
        self.assertTupleEqual(actual.shape, expected.shape, "Shapes not matching")
        np.testing.assert_almost_equal(
            actual, expected, decimal=8, err_msg="big_g output not matching"
        )
    ...
    def test_smith_wilson(self):
        ...
        # Actual output
        actual = solvency2_data.smith_wilson(
            instrument,
            liqmat,
            rates,
            nrofcoup,
        ...
        np.testing.assert_almost_equal(
            ...
            expected,
            decimal=8,
            err_msg="smith_wilson_brute_force output not matching",
        )
